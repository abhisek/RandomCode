...
module Grinder
	module Core
		module Debug
			...
			class SymModManager
				...
				def [](a)
					...
					@process.modules.each do |mod|
						if (mod.addr >= a) and ((mod.addr + mod.size) > a)
							name = mod.path
							break
						end
					end
			...
			ARCH_PATH					 = "x86"
			#ARCH_PATH					 = ::Metasm::WinAPI.host_cpu.size == 64 ? 'x64' : 'x86'
			DBGHELP_PATH                 = "#{DATA_PATH}\\#{ARCH_PATH}\\dbghelp.dll"
			SYMSRV_PATH                  = "#{DATA_PATH}\\#{ ::Metasm::WinAPI.host_cpu.size == 64 ? 'x64' : 'x86' }\\symsrv.dll"
			...
			class ProcessSymbols
				...
				def initialize( process, handle, addrsz )
					@pid    = process.pid
					@handle = handle
					@addrsz = addrsz
					@modules = SymModManager.new(process)
				...
				def address2symbol( address )
					...
					begin
						...
						raise 'Error in symfromaddr' if( not success or success == 0 )
						
						mod_name = @modules[ sym[:modbase] ]
						
						raise 'mod_name not found' if( not mod_name or mod_name.empty? )
						...
						result = ""
						puts "Error while resolving symbol: #{e.message}"
					end
